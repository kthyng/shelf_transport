...
# new version of sp/sn etc from higher resolution
years = np.arange(2004, 2015)
for i, year in enumerate(years):
    d = np.load('calcs/alongcoastconn/conn_in_time/lines_by_start_4hours%i.npz' % year)
    # number of simulations (matching dates) x time in simulation (4 hours) x alongcoast conn box
    if i == 0:
        spfull = d['startingp']; snfull = d['startingn']
    else:
        spfull = np.vstack((spfull, d['startingp']))
        snfull = np.vstack((snfull, d['startingn']))

# ADD ONE to remove self-connectivity, only need to do downcoast
# this was already accounted for in calc_alongcoast_startendintime.py
# snfull[:,0,:] += 1

index = pd.date_range('2004-01-01',end='2014-8-30 00:00', freq='1d')
# indexmonthly = pd.date_range('2004-01-01',end='2014-8-30 00:00', freq='1MS')
#  all of dec 2012 but 12/1/12 is missing
# remove those time indices
irem = [np.where(index == date)[0][0] for date in pd.date_range('2012-12-2', end='2012-12-31', freq='1d')]
index = index.delete(irem)

# # change to monthly to match sp/sn:
# # leave 0 axis, mean of axis 1 across 30 time steps, leave axis 2
# # spfull is # files x 180  x starting box (342) initially, then # files x 342
# spfull2 = spfull.sum(axis=1)/(spfull>0).sum(axis=1)
# sp = pd.DataFrame(index=index, data=spfull2)
# # mean across month
# sp = sp.resample('1MS').mean()
# snfull2 = snfull.sum(axis=1)/(snfull<0).sum(axis=1)
# sn = pd.DataFrame(index=index, data=snfull2)
# sn = sn.resample('1MS').mean()

# CHANGE HERE TO TRY HIGHER RES AND MONTHLY CONNECTIVITIY DISTANCE
spfull2 = (spfull*t[np.newaxis,:,np.newaxis]).sum(axis=1)
spdaily = pd.DataFrame(index=index, data=spfull2)
spmonthly = sp.resample('1MS').mean()

snfull2 = (snfull*t[np.newaxis,:,np.newaxis]).sum(axis=1)  # daily
sndaily = pd.DataFrame(index=index, data=snfull2)
snmonthly = sn.resample('1MS').mean()  # monthly
...
walongmonth = walong.resample('1MS').mean()
wacrossmonth = wacross.resample('1MS').mean()
missmonth = miss.resample('1MS').mean()
atchmonth = atch.resample('1MS').mean()
brazmonth = braz.resample('1MS').mean()
...
cols0 = ['walong','wacross','walong+wacross','miss','atch','braz']
         # 'walong+miss', 'walong+braz',
         #'wacross+miss','wacross+atch', 'wacross+braz',
         #'miss+atch']#, 'miss+atch+braz', 'miss+braz', 'atch+braz','walong+atch']
cols1 = ['%s-r2' % col  for col in cols0]
cols2 = ['%s-p' % col  for col in cols0]
cols3 = ['%s-p2' % col  for col in cols0 if '+' in col]
cols = cols1 + cols2 + cols3

cols0names = ['Wind [along]', 'Wind [across]', 'Wind [both]',
              'Mississippi water', 'Atchafalaya water', 'Brazos water']

iboxes = [20, 77, 276]

ls = {'walong': {'color': '0.6', 'ls': '-'},
      'wacross': {'color': '0.6', 'ls': '--'},
      'walong+wacross': {'color': '0.6', 'ls': ':'},
      'miss': {'color': '#E03D7D', 'ls': '-'},
      'atch': {'color': '#7DE03D', 'ls': '-'},
      'braz': {'color': '#3D7DE0', 'ls': '-'},
}
yminleft = 0.05; ymaxleft = 0.8; ymaxright = 1400
# loop over range of wind angle shifts to see impact on correlations alongcoast
for rot in [0]:# np.arange(0,360,15):  # every 15 deg

    corrspall = pd.DataFrame(index=walong.columns, columns=cols)
    corrsnall = pd.DataFrame(index=walong.columns, columns=cols)
    # loop over coastal boxes
    for ibox in range(1,340):
    ...
    for ip, col in enumerate(cols0):
        ...
        y[~inds] = np.nan
        ax.plot(x, y, lw=2, label=cols0names[ip], **ls[col])
    fig.legend()
    ax.set_xlabel('Along-coast distance [km]')
    ax.set_ylabel('r$^2$')
    # ax.set_ylim(yminleft, ymaxleft)
    ax.set_xlim(0, dist.max())
    # lines for examples
    for ibox in iboxes:
        ax.vlines(dist[ibox], yminleft, ymaxleft, linewidth=0.9, color='0.3', alpha=0.5)
    # also plot overall connectivity
    ax2 = ax.twinx()
    ax2.fill_between(dist, sp.iloc[:,:342].sum(axis=0), color=colu, alpha=0.2)
    ax2.set_ylabel('Connectivity upcoast', color=colu, horizontalalignment='right')
    ax2.tick_params(axis='y', colors=colu)
    # ax2.set_ylim(0, ymaxright)
    ...
    # also plot overall connectivity
    ax2 = ax.twinx()
    ax2.fill_between(dist, abs(sn.iloc[:,:342].sum(axis=0)), color=cold, alpha=0.2)
    ax2.set_ylabel('Connectivity downcoast', color=cold, horizontalalignment='right')
...
for ibox in iboxes:
    ...
    fig.subplots_adjust(left=0.1, bottom=0.23, right=0.97, top=0.94, wspace=0.11)
    if ibox == 20:
        x = 'walong'
        y = 'wacross'
        xlabel = 'Wind (along) [N/m$^2$]'
        ylabel = 'Wind (across) [N/m$^2$]'
        xmax = 0.07
        ymax = 0.05; ymin = -ymax
        yfac = 1
        # text for axes by quadran
        text00 = ()
        text01 = (0.6, 0.95, 'onshore, upcoast')
        text10 = (0.01, 0.02, 'offshore, downcoast')
        text11 = (0.6, 0.02, 'offshore, upcoast')
    elif ibox == 77:
        x = 'walong'
        y = 'atch'
        xlabel = 'Wind (along) [N/m$^2$]'
        ylabel = 'Atchafalaya water [%]'
        xmax = 0.07
        ymax = 11; ymin = -.05
        yfac = 100  # river water is a percent
        text00 = (0.05, 0.02, 'downcoast')
        text01 = (0.75, 0.95, 'upcoast')
        text10 = ()
        text11 = ()
    elif ibox == 276:
        x = 'walong'
        y = 'miss'
        xlabel = 'Wind (along) [N/m$^2$]'
        ylabel = 'Mississippi water [%]'
        xmax = 0.07
        ymax = 80; ymin = 0
        yfac = 100  # river water is a percent
        text00 = (0.05, 0.02, 'downcoast')
        text01 = (0.75, 0.02, 'upcoast')
        text10 = ()
        text11 = ()
    ...
    for c,ax in zip([c0, c1],axes):
        mappable = ax.scatter(dftn[x], dftn[y]*yfac, c=c, s=100, cmap='cmo.curl_r', vmin=-vmax, vmax=vmax)
        ax.hlines(0, -xmax, xmax, linestyle=':')
        ax.vlines(0, ymin, ymax, linestyle=':')
        ax.set_ylim(ymin, ymax); ax.set_xlim(-xmax, xmax);
        # add some text
        for text in [text00, text01, text10, text11]:
            if text != ():
                ax.text(*text, color='0.6', transform=ax.transAxes)
    cax = fig.add_axes([0.175, 0.1, 0.7, 0.025])
    cb = fig.colorbar(mappable, cax=cax, orientation='horizontal')
    cb.set_label('Connectivity')
    fig.text(0.175, 0.02, 'downcoast', color=cold)
    fig.text(0.8, 0.02, 'upcoast', color=colu)
    # add wind stats to plot
    for i, (col, text) in enumerate(zip([x, y], [xlabel, ylabel])):
        result = sm.ols(formula="sp ~ " + col, data=dftp).fit()
        ax = axes[1]
        # positive or negative correlation
        sign = np.sign(np.corrcoef([dftp.loc[:'2011','sp'], dftn.loc[:'2011',col]])[0,1])
        if sign<0:
            s = '-'
        else:
            s = '+'
        ax.text(0.01, 0.95-i*0.2, '%s' % text, transform=ax.transAxes)
        ax.text(0.01, 0.9-i*0.2, 'r$^2$=%0.2f ($%s$)' % (result.rsquared_adj, s), transform=ax.transAxes)
        ax.text(0.01, 0.85-i*0.2, 'p=%1.1e' % result.pvalues[1], transform=ax.transAxes)
        # if '+' in col:
        #     ax.text(0.01, 0.8-i*0.2, 'p2=%0.2f' % result.pvalues[2], transform=ax.transAxes)

        # include negative since magnitude increases away from zero
        result = sm.ols(formula="sn ~ " + col, data=dftn).fit()
        ax = axes[0]
        # positive or negative correlation
        sign = np.sign(np.corrcoef([-dftn.loc[:'2011','sn'], dftn.loc[:'2011',col]])[0,1])
        if sign<0:
            s = '-'
        else:
            s = '+'
        ...
        # if '+' in col:
        #     ax.text(0.01, 0.8-i*0.2, 'p2=%0.2f' % result.pvalues[2], transform=ax.transAxes)
    fig.suptitle('Distance: %2d km' % (dist[ibox]))
    fig.savefig('%s/ibox_%i.png' % (base, ibox))
    fig.savefig('%s/ibox_%i.pdf' % (base, ibox))
    plt.close(fig)
...
#     else:
#         sp5 = np.vstack((sp5, d['startingp']))
#         sn5 = np.vstack((sn5, d['startingn']))
#
# # ADD ONE to remove self-connectivity, only need to do downcoast
# sn5[:,0,:] += 1
#
...
# # sum across month
# sp6 = pd.DataFrame(index=index, data=sp5.sum(axis=1))
# sp6 = sp6.resample('1MS').sum()
# sn6 = pd.DataFrame(index=index, data=sn5.sum(axis=1))
# sn6 = sn6.resample('1MS').sum()
# # change to monthly to match sp/sn:
# # leave 0 axis, sum of axis 1, leave axis 2
# # mean across month
# sp7 = pd.DataFrame(index=index, data=sp5.sum(axis=1))
# sp7 = sp7.resample('1MS').mean()
# sn7 = pd.DataFrame(index=index, data=sn5.sum(axis=1))
# sn7 = sn7.resample('1MS').mean()
# # change to monthly to match sp/sn:
# # leave 0 axis, mean of axis 1, leave axis 2
# # mean across month
# sp8 = pd.DataFrame(index=index, data=sp5.mean(axis=1))
# sp8 = sp8.resample('1MS').mean()
# sn8 = pd.DataFrame(index=index, data=sn5.mean(axis=1))
# sn8 = sn8.resample('1MS').mean()
# # change to monthly to match sp/sn:
# # leave 0 axis, mean of axis 1, leave axis 2
# # mean across month
# sp9 = pd.DataFrame(index=index, data=sp5.mean(axis=1))
# sp9 = sp9.resample('1MS').sum()
# sn9 = pd.DataFrame(index=index, data=sn5.mean(axis=1))
# sn9 = sn9.resample('1MS').sum()
#
# ibox = 150
#
# which = 'try9'
# # THE COMPARISON BETWEEN original and try5 is almost identical but slightly
# # different because of my now accounting for the single extra -1 in sn5 at
# # the first time step
#
...
# elif which == 'try5':  # THIS ONE MATCHES
#     x = walongmonth.loc[:,str(ibox)]
#     y = wacrossmonth.loc[:,str(ibox)]
#     z = sn7[ibox]
...
#     y = wacrossmonth.loc[:,str(ibox)]
#     z = sn9[ibox]
#     title = 'trying to match, with mean/sum'
# elif which == 'try8':
#     x = walong2.loc[:,str(ibox)]
#     y = wacross2.loc[:,str(ibox)]
#     ind = np.where(index == walong2.index[-1])[0][0]
#     z = sn5[:ind+1,:,ibox].sum(axis=1)
#     title = 'compare start day of sim to wind that day'
# elif which == 'try9':
#     # do comparison by start day but average wind over 30 days of simulation in a rolling sense
#     x = walong2.loc[:,str(ibox)].rolling(window=30).mean()
#     y = wacross2.loc[:,str(ibox)].rolling(window=30).mean()
#     ind = np.where(index == walong2.index[-1])[0][0]
#     z = sn5[:ind+1,:,ibox].sum(axis=1)
#     title = 'transport sum per simulation with mean wind across sim'
#
# vmax = abs(np.nanmin(z))
#
# fig = plt.figure()
